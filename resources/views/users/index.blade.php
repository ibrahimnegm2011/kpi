...
<x-app-layout>
    <div class="w-full mt-1">
        <div class="flex justify-between mb-5">
            <h1 class="text-3xl text-black"><i class="fas fa-users mr-3"></i> Users </h1>
            <div class="flex gap-3">
                <a class="bg-primary-500 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded"
                   href="{{route('users.invite')}}">Invite Representable</a>
            ...
            <form method="get" id="search-form">
                ...
                <div class="flex gap-3 mt-5">
                    ...
                    <div class="inline-flex rounded-md shadow-sm" role="group">
                        <button type="button"
                                class="px-4 py-2 border border-primary-300 text-primary-700 bg-white hover:bg-primary-50 focus:z-10 focus:ring-2 focus:ring-primary-600 {{ request('filter.type') === null || request('filter.type') === '' ? 'bg-primary-100' : '' }}"
                                onclick="setType('')">
                            All
                        </button>
                        <button type="button"
                                class="px-4 py-2 border-t border-b border-primary-300 text-primary-700 bg-white hover:bg-primary-50 focus:z-10 focus:ring-2 focus:ring-primary-600 {{ request('filter.type') === 'normal' ? 'bg-primary-100' : '' }}"
                                onclick="setType('normal')">
                            Normal
                        </button>
                        <button type="button"
                                class="px-4 py-2 border-t border-b border-l border-primary-300 text-primary-700 bg-white hover:bg-primary-50 focus:z-10 focus:ring-2 focus:ring-primary-600 {{ request('filter.type') === 'representative' ? 'bg-primary-100' : '' }}"
                                onclick="setType('representative')">
                            Representative
                        </button>
                        <button type="button"
                                class="px-4 py-2 border border-primary-300 text-primary-700 bg-white hover:bg-primary-50 focus:z-10 focus:ring-2 focus:ring-primary-600 {{ request('filter.type') === 'admin' ? 'bg-primary-100' : '' }}"
                                onclick="setType('admin')">
                            Admin
                        </button>
                        <input type="hidden" name="filter[type]" id="filterType" value="{{ request('filter.type') }}">
                    </div>
            ...
            <table class="w-full border-collapse text-left">
                ...
                @foreach($users as $user)
                    <tr class="hover:bg-gray-100 {{$user->is_admin ? 'bg-primary-50': ''}}" title="Admin User">
                        ...
                        <td class="py-4 px-6 border-b border-grey-light">{{$user->company?->name}}</td>
                        <td class="py-4 px-6 border-b border-grey-light">{{$user->department?->name}}</td>
                        <td class="py-4 px-6 border-b border-grey-light">{{$user->is_active? 'Active': 'Inactive'}}</td>
                        <td class="py-4 px-6 border-b border-grey-light ">
                            @if(Auth::id() != $user->id)
                                <a class="mx-2 text-secondary-500 hover:text-primary-500" href="{{route('users.edit', $user->id)}}"> <i class="fas fa-edit"></i> </a>
                                <form method="POST" action="{{ route('users.delete', $user) }}" style="display:inline;"
