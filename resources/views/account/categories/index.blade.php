<x-app-layout>
    <div class="w-full mt-1">
        <div class="flex justify-between mb-5">
            <h1 class="text-3xl text-black"><i class="fas fa-list mr-3"></i> Categories </h1>
            <div class="flex gap-3">
                <a class="bg-secondary-500 hover:bg-secondary-700 text-white font-bold py-2 px-4 rounded"
                   href="{{route('account.categories.create')}}">Add Category</a>
            </div>
        </div>
                ...
                @foreach($categories as $category)
                    <tr class="hover:bg-gray-100">
                        ...
                        <td class="py-4 px-6 border-b border-grey-light ">
                            <a class="mx-2 text-secondary-500 hover:text-primary-500" href="{{route('account.categories.edit', $category->id)}}"> <i class="fas fa-edit"></i> </a>
                            <form method="POST" action="{{ route('account.categories.delete', $category) }}" style="display:inline;"
    ...
    <x-slot:scripts>
        <script>
            ...
            function confirmDelete(categoryName) {
                return confirm(`Are you sure you want to delete category "${categoryName}"? This action cannot be undone.`);
            }
        </script>
    </x-slot:scripts>
</x-app-layout>
