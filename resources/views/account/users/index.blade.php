        ...
        <div class="flex justify-between mb-5">
            ...
            <div class="flex gap-3">
                <a class="bg-secondary-500 hover:bg-secondary-700 text-white font-bold py-2 px-4 rounded"
                   href="{{route('account.users.create')}}">Add User</a>
            </div>
        </div>
            ...
            <table class="w-full border-collapse text-left">
                ...
                @if(! $users->count())
                    <tr class="hover:bg-gray-100">
                        <td colspan="5" class="py-4 px-6 border-b border-grey-light"> No users found. </td>
                    </tr>
                @endif
                @foreach($users as $user)
                    <tr class="hover:bg-gray-100">
                        ...
                        <td class="py-4 px-6 border-b border-grey-light ">
                            <a class="mx-2 text-secondary-500 hover:text-primary-500" href="{{route('account.users.edit', $user->id)}}"> <i class="fas fa-edit"></i> </a>
                            <form method="POST" action="{{ route('account.users.delete', $user) }}" style="display:inline;"
