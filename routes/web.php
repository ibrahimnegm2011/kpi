...
use App\Http\Controllers\Representatives\KpisController as RepresentativeKpisController;
...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('users')->name('users.')->controller(UsersController::class)->middleware('can:'.Permission::USERS())->group(function () {
        ...
        Route::get('/invite', 'invite_form')->name('invite');
        Route::post('/send-invitation', 'invite')->name('send_invitation');

        Route::get('/onboarding/{user}', 'onboarding_form')->name('onboarding.show');
        Route::post('/onboarding/{user}', 'onboarding')->name('onboarding');

        Route::delete('/{user}', 'delete')->name('delete');
    });

    Route::middleware(['representative'])->namespace('App\Http\Controllers\Representatives')->group(function () {
        Route::get('/our-kpis',[ RepresentativeKpisController::class, 'index'])->name('company_kpis');
        Route::get('/done-kpis',[ RepresentativeKpisController::class, 'submitted'])->name('submitted_kpis');
        Route::get('/overdue-kpis',[ RepresentativeKpisController::class, 'overdue'])->name('overdue_kpis');
        Route::get('/submit/{forecast}',[ RepresentativeKpisController::class, 'form'])->name('kpi_submit_form');
        Route::post('/submit/{forecast}',[ RepresentativeKpisController::class, 'submit'])->name('kpi_submit');
    });
